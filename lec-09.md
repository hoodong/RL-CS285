...
- p4: 아래 식이 성립하는 이유는?
  ...
  - 왜냐하면 $V^{\pi_\theta}(s_0)$에서 random한 부분은 $s_0$ 뿐이므로 $a_t$가 어떤 정책이든 기대값 계산에 영향을 주지 않음  
    즉, $E_{s_0}[V^{\pi_\theta}(s_0)]=E_{s_0,a_0,s_1,...}[V^{\pi_\theta}(s_0)]$   
    강의자료에서는 수식 전개를 위해 $a_t$가 new policy를 따른다고 가정
