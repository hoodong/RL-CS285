...
- p8: 가치함수 $\hat{V}_{\phi}^{\pi}$ 학습 (MC)
  - 데이터: $\lbrace (s_{i,t},y_{i,t}) \rbrace$, where
    $y_{i,t}=\sum\limits_{t'=t}^{T}r(s_{i,t'},a_{i,t'})$: MC target
  - 손실함수: $L(\phi)=\frac{1}{2}\sum_i||\hat{V} _\phi^\pi(s_i)-y_i||^2$
- p9: TD target  $y_{i,t}=r(s_{i,t},a_{i,t})+\hat{V}_ {\phi}^{\pi}(s_{i,t+1})$
